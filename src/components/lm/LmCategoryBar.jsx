...
export default function LmCategoryBar({ active, filter = 'all', onFilter, showFilter = true, filters = FILTERS, title }) {
  ...
  return (
    <div className="sticky top-[70px] z-40 border-b border-white/40 bg-white/55 shadow-[0_8px_32px_rgba(15,15,17,0.06)] backdrop-blur-2xl backdrop-saturate-150">
      {/* top-edge light catch — the "glass lip" */}
      <span className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/90 to-transparent" aria-hidden="true" />
      {/* Scrollable on small screens so the pill row never forces page overflow */}
      <div className="mx-auto flex w-full max-w-[1440px] items-center gap-[14px] overflow-x-auto px-4 py-[12px] [scrollbar-width:none] sm:gap-[20px] sm:py-[14px] sm:px-8 lg:px-[32px] [&::-webkit-scrollbar]:hidden">
        {title && (
          <h1 className="shrink-0 whitespace-nowrap text-[19px] font-bold tracking-[-0.01em] text-[#0F0F11] sm:text-[22px]" style={serif}>
            {title}
            <span className="ml-[6px] inline-block size-[6px] animate-pulse rounded-full bg-[#3979FF] align-middle" aria-hidden="true" />
          </h1>
        )}
        <div className="relative flex shrink-0 items-center gap-[2px] rounded-[100px] border border-white/60 bg-white/50 p-[4px] shadow-[inset_0_1px_0_rgba(255,255,255,0.8),inset_0_-1px_0_rgba(15,15,17,0.04),0_2px_12px_rgba(15,15,17,0.06)] backdrop-blur-xl sm:gap-[4px] sm:p-[5px]">
