...
function Excerpt({ item, size = 'lg' }) {
  ...
  return (
    <p className={`font-roboto ${cls} text-lm-500`} style={rb}>
      ...
      {text.length > limit && (
        <button type="button" onClick={() => setOpen(!open)} className="font-roboto font-medium text-lm-700 hover:underline" style={rb}>
...
function FeaturedCard({ item, lead = false, half = false }) {
  return (
    <article
      className={`flex flex-col gap-[24px] rounded-[16px] bg-white p-[20px] sm:p-[32px] ${
    ...
    >
      ...
      <Excerpt item={item} size={half ? 'sm' : 'lg'} />
      <SourceRow item={item} />
    </article>
  )
}
...
function CompactCard({ item }) {
  return (
    <article className="flex flex-col gap-[8px] rounded-[16px] border border-[rgba(28,28,30,0.1)] bg-white p-[16px]">
      <h3 className="font-roboto text-[21px] font-semibold leading-[1.36] text-black" style={rb}>{item.headline}</h3>
      <Excerpt item={item} size="sm" />
      <SourceRow item={item} />
    </article>
  )
}
      ...
      <div className="flex flex-col gap-[8px]">
        ...
        {(halfA || halfB) && (
          <div className="flex flex-col gap-[8px] sm:flex-row">
            {halfA && <div className="flex-1"><FeaturedCard item={halfA} half /></div>}
            {halfB && <div className="flex-1"><FeaturedCard item={halfB} half /></div>}
          </div>
        )}
      </div>
      {compacts.length > 0 && (
        <div className="flex flex-col gap-[8px]">
          {compacts.map((c) => <CompactCard key={c.id} item={c} />)}
        </div>
      )}
    </div>
  )
        ...
        >
          ...
          <div className="flex flex-col gap-[8px]">
            {Array.from({ length: Math.ceil(g.items.length / 7) }, (_, i) => (
              <Block key={i} items={g.items.slice(i * 7, i * 7 + 7)} />
            ))}
          </div>
        </motion.section>
      ))}
