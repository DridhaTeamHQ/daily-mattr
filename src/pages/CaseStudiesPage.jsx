  ...
  return (
    ...
    >
      <Link
        ...
        className="group flex h-full flex-col rounded-2xl border border-gray-200 bg-white p-6 transition-all hover:-translate-y-0.5 hover:border-gray-300 hover:shadow-[0_16px_36px_-28px_rgba(17,24,39,0.4)]"
      >
        <span
        ...
        <h3 className="mt-4 text-[20px] font-bold leading-snug text-gray-900 decoration-[#7b1e3b]/50 decoration-2 underline-offset-4 group-hover:underline" style={SERIF}>
          {item.headline}
        </h3>
        <p className="mt-2 line-clamp-4 flex-1 text-[14px] leading-relaxed text-gray-600" style={SANS}>
          {item.summary}
        </p>
        <div className="mt-5 flex items-center justify-between text-[12px] text-gray-500" style={SANS}>
          <span>{item.source || 'Daily Mattr'} · {readTime(item.body, item.summary)} min</span>
        ...
        <div className="min-w-0 overflow-x-clip lg:col-start-2 lg:row-start-1">
          <SwipeCard id={item.id} enterX={enterX} reduce={reduce} onNext={goNext} onPrev={goPrev}>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3 text-[13px] text-gray-500" style={SANS}>
              ...
              {item.source && item.publishedAt && <span aria-hidden>·</span>}
              {item.publishedAt && <span>{dateLabel(item.publishedAt)}</span>}
              <span aria-hidden>·</span>
              <span>{readTime(item.body, item.summary)} min read</span>
            </div>

            <div className="desi-divider my-7"><span className="desi-divider__motif" style={SANS}>❖</span></div>

            <div className="article-body space-y-5 text-[17px] leading-[1.8] text-gray-700" style={SANS}>
              {paras.map((p, i) => (
